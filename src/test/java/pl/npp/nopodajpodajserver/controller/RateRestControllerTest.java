...
public class RateRestControllerTest {
    ...
    public void deleteRate() throws Exception {
        mockMvc.perform(delete("/rates/2")).andExpect(status().isOk());
    }

    @Test
    public void findByPlaceId() throws Exception {
        mockMvc.perform(get("/rates/byPlace/1")).andExpect(status().isOk())
                .andExpect(content().contentType(contentType));
    }

    @Test
    public void findByCustomerId() throws Exception {
        mockMvc.perform(get("/rates/byCustomer/1")).andExpect(status().isOk())
                .andExpect(content().contentType(contentType));
    }
